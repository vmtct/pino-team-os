...
export function closeoutGuidanceState(board: StaffDutyBoard | null) {
  if (!board || board.gate.action !== "CHECK_OUT") {
    return { ready: false, ambiguous: true, outstanding: [], approvedException: false } as const;
  }
  ...
  const approvedException = board.checkoutException?.status === "APPROVED";
  return {
    ready: !ambiguous && (outstanding.length === 0 || approvedException),
    ambiguous,
    outstanding,
    approvedException,
  };
