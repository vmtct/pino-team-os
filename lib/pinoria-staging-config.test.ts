...
test("Pinoria Team staging is isolated from production Core", () => {
  ...
  assert.equal(staging.services.length, 9);
  ...
  assert.deepEqual(staging.services.find((service: { binding: string }) => service.binding === "PINO_STAFF_PASSWORD_CORE"), { binding: "PINO_STAFF_PASSWORD_CORE", service: "pino-core-staging", entrypoint: "StaffPasswordControlPlane" });
  assert.deepEqual(staging.services.find((service: { binding: string }) => service.binding === "PINO_WORKFORCE_NETWORK_CORE"), { binding: "PINO_WORKFORCE_NETWORK_CORE", service: "pino-core-staging", entrypoint: "WorkforceNetworkPresenceControlPlane" });
