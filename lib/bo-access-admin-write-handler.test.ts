...
test("BO access admin facade forwards only whitelisted role lifecycle and assignment POSTs with verified identity", async () => {
  const f = await fixture();
  const forwarded: Array<{ request: BoAccessRequest; identity: VerifiedBoIdentity }> = [];
  const binding: BoAccessCoreBinding = { async execute(coreRequest, identity) {
  ...
  const cases = [
    ["access/roles", roleBody],
    [`access/roles/${roleId}/duplicate`, duplicateBody],
    [`access/roles/${roleId}/update`, updateBody],
    [`access/roles/${roleId}/archive`, {}],
    ["access/assignments", assignmentBody],
  ] as const;
  for (const [path, body] of cases) {
    const response = await handleBoWriteRequest(request(path, f.token, body), env(binding), path, f.resolver);
    assert.equal(response.status, 200);
  }
  assert.deepEqual(forwarded.map((item) => item.request), cases.map(([path, body]) => ({ method: "POST", path, body })));
...
test("access admin writes require BO Access identity and reject unlisted write paths before Core", async () => {
  ...
  const [unauthenticated, unknown, deleteRole] = await Promise.all([
    ...
    handleBoWriteRequest(request(`access/roles/${roleId}/delete`, f.token), env(binding), `access/roles/${roleId}/delete`, f.resolver),
  ]);
