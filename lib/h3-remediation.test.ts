...
test('Access perimeter retirement recovery is full-state CAS and receipt-safe',()=>{for(const t of [perimeter,watchdog]){assert.match(t,/Original app state b64/); assert.match(t,/Desired app state b64/); assert.match(t,/\.policies=\(\(\.policies \/\/ \[\]\)/);} assert.match(perimeter,/post-mutation full state does not equal the preserved desired state/); assert.match(perimeter,/restored_state.*original_state/); assert.match(watchdog,/Access-app rollback payload does not match captured baseline state/); assert.match(watchdog,/Access-app mutation payload does not match captured desired state/); assert.match(watchdog,/rollback PUT did not return a successful provider result/); assert.match(watchdog,/rollback PUT completed but exact baseline restoration was not proven/); assert.match(watchdog,/live full state no longer equals this run's desired state or baseline/);});
test('Access perimeter expected state includes deterministic Cloudflare rehome normalization',()=>{for(const t of [perimeter,watchdog,recoveryConfirm]){assert.match(t,/expected_rehomed_state/); assert.match(t,/self_hosted_domains/); assert.match(t,/destinations/); assert.match(t,/\.name=\$to/); assert.match(t,/\.domain=\$to/);} assert.match(perimeter,/desired_state=.*expected_rehomed_state/); assert.match(watchdog,/expected_desired_state=.*expected_rehomed_state/);});
