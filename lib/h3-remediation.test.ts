...
test('Access perimeter expected state includes deterministic Cloudflare rehome normalization',()=>{for(const t of [perimeter,watchdogApp,recoveryConfirm]){assert.match(t,/expected_rehomed_state/); assert.match(t,/self_hosted_domains/); assert.match(t,/destinations/); assert.match(t,/\.name=\$to/); assert.match(t,/\.domain=\$to/); assert.match(t,/\.http_only_cookie_attribute=true/); assert.doesNotMatch(t,/if \(\.name \/\/ \"\\\"\) == \$from/);} assert.match(perimeter,/desired_state=.*expected_rehomed_state/); assert.match(watchdogApp,/expected_desired_state=.*expected_rehomed_state/);});
