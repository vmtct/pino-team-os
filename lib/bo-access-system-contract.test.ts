...
test("BO Access System facade exposes only bounded administration paths", () => {
  for (const readPath of ["access/users", "access/roles", "access/permissions", "access/audit", `access/roles/${roleId}`]) {
    assert.equal(isOperationalReadPath(readPath), true, readPath);
  }
  for (const writePath of ["access/roles", `access/roles/${roleId}/duplicate`, `access/roles/${roleId}/update`, `access/roles/${roleId}/archive`, "access/assignments", "access/assignments/remove", "access/users/status"]) {
    assert.equal(isAllowedPostPath(writePath), true, writePath);
  }
...
test("BO shell places Access administration under System Users Roles Audit", () => {
  const source = fs.readFileSync(path.join(process.cwd(), "app/bo/layout.tsx"), "utf8");
