...
export const boApi = {
  ...
  transferEnrollment: (enrollmentId: string, body: { destinationRunningClassId: string; transferLocalDate: string; plannedEntryLocalTime?: string | null; plannedDurationMinutes?: number | null; commandEffectiveLocalDate: string; policyEffectiveAt: string; reason: string }, idempotencyKey: string) => write<unknown>(`enrollments/${encodeURIComponent(enrollmentId)}/transfer`, body, idempotencyKey),
  resetParentPin: (parentUserId: string, idempotencyKey: string) => write<{ command: string; temporaryPin: string | null; expiresAt: string; credentialVersion: number; replayed: boolean }>(`identity/parents/${encodeURIComponent(parentUserId)}/pin/reset`, {}, idempotencyKey),
