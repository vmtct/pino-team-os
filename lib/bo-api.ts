...
export const boApi = {
  ...
  resetParentPin: (parentUserId: string, idempotencyKey: string) => write<{ command: string; temporaryPin: string; expiresAt: string; credentialVersion: number }>(`identity/parents/${encodeURIComponent(parentUserId)}/pin/reset`, {}, idempotencyKey),
