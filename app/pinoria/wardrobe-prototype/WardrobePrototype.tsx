...
const footer = [
  { id: "home", label: "Home", href: "/dashboard" }, { id: "presence", label: "Hiện diện", href: "/pinoria" },
  { id: "attendance", label: "Điểm danh", href: "/pinoria/attendance" }, { id: "wardrobe", label: "Tủ đồ", href: "/pinoria/wardrobe-prototype" },
];
...
export function WardrobePrototype() {
  ...
  if (!learner || !state) return <LearnerPicker query={learnerQuery} setQuery={setLearnerQuery} rows={learnerRows} invalid={Boolean(learnerId)} choose={(id) => router.push(`/pinoria/wardrobe-prototype?learnerId=${id}`)} />;
  return <TosShell title="Tủ đồ Pinoria" subtitle="Prototype · local fixture · staff operator" theme="pinoria" footerItems={footer} activeFooterId="wardrobe" backHref="/pinoria">
    <div className={styles.page}>
      <section className={styles.learnerBar}><button className={styles.learnerIdentity} onClick={() => router.push("/pinoria/wardrobe-prototype")}><span>{learner.name.charAt(0)}</span><div><strong>{learner.name}</strong><small>{learner.meta} · Đổi học viên</small></div><b>⌄</b></button><button className={styles.grantButton} onClick={() => setGrantOpen(true)}>＋ Cấp món</button></section>
...
function LearnerPicker({ query, setQuery, rows, invalid, choose }: { query: string; setQuery: (value: string) => void; rows: Learner[]; invalid: boolean; choose: (id: string) => void }) {
  return <TosShell title="Tủ đồ Pinoria" subtitle="Chọn học viên để quản lý Wardrobe" theme="pinoria" footerItems={footer} activeFooterId="wardrobe" backHref="/pinoria"><div className={styles.page}><section className={styles.pickerIntro}><span className={styles.eyebrow}>PNR-WARD · PROTOTYPE</span><h2>Wardrobe của ai?</h2><p>Prototype dùng local fixture. Không gọi Core và không thay đổi dữ liệu thật.</p></section>{invalid ? <div className={styles.warning}>Learner trong URL không tồn tại trong fixture. Hãy chọn lại.</div> : null}<input className={styles.search} autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm tên học viên…"/><section className={styles.learnerList}>{rows.map((learner) => <button key={learner.id} onClick={() => choose(learner.id)}><span>{learner.name.charAt(0)}</span><div><strong>{learner.name}</strong><small>{learner.meta} · {learner.state.owned.length} món sở hữu</small></div><b>→</b></button>)}</section></div></TosShell>;
}
