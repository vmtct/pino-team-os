...
import { callTosLearningCoreWithStaffPin, type TosLearningCoreBinding } from "@/lib/tos-learning-core";
...
async function handle(request: Request, context: Context) {
  try {
    ...
    const staffToken = readStaffSessionToken(request);
    if (!staffToken) return missingStaffSessionResponse();
    ...
    const result = await callTosLearningCoreWithStaffPin(
