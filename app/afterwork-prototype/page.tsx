...
type Booking = { id:string; name:string; party:string; session:string; status:BookingStatus; holdUntil?:string; paymentIssue?:string };
type Session = { id:string; title:string; kind:"Acrylic"|"Piano Spa"; day:string; time:string; booked:number; capacity:number; facilitator:string; deposit:string };

const sessions:Session[]=[
  {id:"s1",title:"Sunday Flowers",kind:"Acrylic",day:"Sat 22 Aug",time:"14:30",booked:4,capacity:8,facilitator:"Trang",deposit:"100k / seat"},
  {id:"s2",title:"Always With Me",kind:"Piano Spa",day:"Sat 22 Aug",time:"19:00",booked:3,capacity:5,facilitator:"Hương",deposit:"100k / seat"},
  {id:"s3",title:"Rainy Window",kind:"Acrylic",day:"Sun 23 Aug",time:"14:30",booked:6,capacity:8,facilitator:"Trang",deposit:"100k / seat"},
  {id:"s4",title:"Kiss The Rain",kind:"Piano Spa",day:"Sun 23 Aug",time:"19:00",booked:4,capacity:5,facilitator:"Bảo",deposit:"100k / seat"},
];
...
const seed:Booking[]=[
  ...
  {id:"b3",name:"Linh",party:"1 adult + child 9",session:"Rainy Window",status:"holding",holdUntil:"10:50"},
  {id:"b4",name:"Huy",party:"1 adult",session:"Rainy Window",status:"confirmed"},
  {id:"b5",name:"Mai",party:"1 adult",session:"Kiss The Rain",status:"expired",paymentIssue:"Late payment received · reconciliation"},
  {id:"b6",name:"Khoa",party:"1 adult",session:"Always With Me",status:"holding",holdUntil:"10:42"},
];
...
function Weekend({totals,holds,confirmed}:{totals:{booked:number;capacity:number};holds:number;confirmed:number}){return <>
  <div className="stats"><Stat n="4" label="Sessions"/><Stat n={`${totals.booked}/${totals.capacity}`} label="Live seats"/><Stat n={String(holds)} label="Active 15m holds"/><Stat n={String(confirmed)} label="Confirmed / attended"/></div>
  <section className="panel"><div className="panel-head"><div><p className="kicker">22–23 August</p><h2>This weekend</h2></div><button>+ New session</button></div><div className="session-list">{sessions.map(s=><article key={s.id}><div className={`kind ${s.kind==="Acrylic"?"paint":"piano"}`}>{s.kind}</div><div><small>{s.day}</small><h3>{s.title}</h3><p>{s.time} · {s.facilitator} · deposit {s.deposit}</p></div><div className="seat"><b>{s.booked}/{s.capacity}</b><span>{s.capacity-s.booked===1?"1 seat left":`${s.capacity-s.booked} seats left`}</span></div><button className="ghost">Open</button></article>)}</div></section>
  <div className="two"><section className="panel"><div className="panel-head"><div><p className="kicker">Attention</p><h2>Needs a look</h2></div></div><ul className="attention"><li><b>Linh · Rainy Window</b><span>HOLDING · expires {seed[2].holdUntil}</span></li><li><b>Khoa · Always With Me</b><span>HOLDING · expires {seed[5].holdUntil}</span></li><li><b>Mai · Kiss The Rain</b><span>Late payment after EXPIRED · reconciliation</span></li></ul></section><section className="panel calm"><p className="kicker">Operating principle</p><h2>Held is not confirmed.</h2><p>Hold TTL is 15 minutes. Late payment only confirms if Core can atomically reacquire capacity; otherwise staff sees reconciliation/refund work instead of an overbook.</p></section></div>
</>}
function Stat({n,label}:{n:string;label:string}){return <div className="stat"><strong>{n}</strong><span>{label}</span></div>}
function Sessions(){return <section className="panel"><div className="panel-head"><div><p className="kicker">Operational catalog → session</p><h2>Sessions</h2></div><button>+ New session</button></div><div className="form-mock"><label>Experience<select defaultValue="Acrylic"><option>Acrylic</option><option>Piano Spa</option></select></label><label>Offering<select defaultValue="Rainy Window"><option>Rainy Window</option><option>Sunday Flowers</option><option>Always With Me</option><option>Kiss The Rain</option></select></label><label>Date<input value="2026-08-23" readOnly/></label><label>Time<input value="14:30 → 17:00" readOnly/></label><label>Capacity<input value="8" readOnly/></label><label>Deposit / seat<input defaultValue="100,000 VND"/></label><label>Hold TTL<input value="15 minutes" readOnly/></label><label>Cancellation<input value="100% refund ≥24h · non-refundable <24h" readOnly/></label><label>Facilitator<select defaultValue="Trang"><option>Trang</option><option>Hương</option><option>Bảo</option></select></label><button>Publish session</button></div><p className="mock-note">100k/seat is the launch prefill, editable before publish. Runtime will snapshot price + deposit terms on every booking through Core.</p></section>}
function Bookings({bookings}:{bookings:Booking[]}){return <section className="panel"><div className="panel-head"><div><p className="kicker">One canonical lifecycle</p><h2>Bookings</h2></div><button>+ Manual booking</button></div><div className="table"><div className="tr th"><span>Name</span><span>Party</span><span>Session</span><span>Status</span></div>{bookings.map(b=><div className="tr" key={b.id}><b>{b.name}</b><span>{b.party}</span><span>{b.session}</span><span className={statusClass(b.status)}>{statusLabel(b.status)}{b.holdUntil?` · until ${b.holdUntil}`:""}{b.paymentIssue?" · RECONCILE":""}</span></div>)}</div><p className="mock-note">Refund policy: customer cancellation ≥24h → 100% deposit refund; &lt;24h/no-show → deposit non-refundable; PINO cancellation → 100% refund.</p></section>}
function Checkin({bookings,toggle}:{bookings:Booking[];toggle:(id:string)=>void}){return <section className="panel check"><div className="panel-head"><div><p className="kicker">Sun · 14:30</p><h2>Rainy Window</h2><p>Confirmed participants only</p></div><span className="big-seat">{bookings.length} eligible</span></div>{bookings.map(b=><button key={b.id} onClick={()=>toggle(b.id)} className={b.status==="attended"?"checked":""}><span className="tick">{b.status==="attended"?"✓":""}</span><b>{b.name}</b><small>{b.party} · {statusLabel(b.status)}</small></button>)}<p className="mock-note">HOLDING and EXPIRED/reconciliation bookings never enter check-in until a valid Core confirmation exists.</p></section>}
