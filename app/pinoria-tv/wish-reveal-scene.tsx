...
import { PrototypeCharacter, type PrototypeCharacterLayerOverrides } from "./prototype-assets";
...
function layerOverrides(config: Record<string, string>): PrototypeCharacterLayerOverrides {
  return {
    back: config.back ?? null,
    body: config.outfit ?? config.body ?? null,
    hair: config.hair ?? null,
    face: config.face ?? null,
    headwear: config.headwear ?? null,
    eyewear: config.eyewear ?? null,
  };
}

function imageUrl(item?: WishRevealItem) {
  return item?.layerAssetKey?.startsWith("http") ? item.layerAssetKey : null;
}
...
function PullVisual({ pull }: { pull: WishRevealPull }) {
  if (pull.wearables.length > 1) {
    return <div className={styles.itemFan}>{pull.wearables.map((item) => {
      ...
      return src ? <img key={item.id} src={src} alt="" draggable={false} /> : <span key={item.id} className={styles.itemGlyph}>✦</span>;
    })}</div>;
  }
  const src = imageUrl(pull.wearables[0]);
  return src ? <img src={src} alt="" draggable={false} /> : <span className={styles.itemGlyph}>{pull.revealKind === "DUPLICATE" ? "◇" : "✦"}</span>;
}
...
function PullCard({ pull, index }: { pull: WishRevealPull; index: number }) {
  ...
  >
    <div className={styles.pullTop}>
      ...
      <span className={styles.rarity}>{pull.rarity === "MYTHIC" ? "HUYỀN THOẠI" : pull.rarity === "RARE" ? "HIẾM" : "THƯỜNG"}</span>
    </div>
...
export function WishRevealScene({ reveal }: { reveal: WishRevealProjection }) {
  ...
  const heroUrl = reveal.banner.heroAssetKey.startsWith("http") ? reveal.banner.heroAssetKey : null;
  ...
  return <section className={styles.scene} data-pinoria-wish-reveal data-wish-reveal-id={reveal.revealId} data-wish-pull-count={reveal.pulls.length} data-perfect={perfect ? "true" : "false"}>
    <div className={styles.mist} /><div className={styles.vignette} />
    <header className={styles.header}>
      <div><span className={styles.eyebrow}>PINORIA · GIEO ƯỚC</span><strong className={styles.bannerName}>{reveal.banner.displayName}</strong></div>
      <span className={styles.region}>{reveal.banner.regionKey.replaceAll("-", " ")}</span>
    </header>
    <div className={styles.layout}>
      <section className={styles.memory}>
        <div className={styles.halo} />
        {heroUrl ? <img className={styles.heroImage} src={heroUrl} alt="" draggable={false} /> : <div className={styles.bearerMark}>{initial}</div>}
        <div className={styles.bearerCaption}><span>ORIGINAL BEARER</span><strong>{reveal.banner.bearer.displayName}</strong><small>{reveal.banner.bearer.title}</small></div>
      </section>
      <section className={styles.results}>
        <div className={styles.learnerBadge}>
          <div className={styles.learnerMini}><PrototypeCharacter subjectId={reveal.subject.studentProfileId} layerOverrides={layerOverrides(reveal.subject.character.config)} motion="celebrate" size="100%" /></div>
          <div><span>ƯỚC CỦA</span><strong>{reveal.subject.displayName}</strong></div>
        </div>
        <p className={styles.story}>{reveal.banner.storyHook}</p>
        <h1 className={styles.resultTitle}>{revealTitle(reveal)}</h1>
        <div className={styles.resultMeta}>
          <span className={`${styles.chip} ${featured ? styles.chipGold : ""}`}>{featured ? `Cộng Hưởng · C${Math.max(0, lastPull?.resonanceAfter ?? 0)}` : "Wearable Memory"}</span>
          <span className={styles.chip}>{reveal.banner.signatureSet.displayName} · {lastPull?.setProgressAfter.owned ?? 0}/3</span>
