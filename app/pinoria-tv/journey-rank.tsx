...
function RankArtwork({ src, size }: { src: string; size: number }) {
  return <span style={{ position: "relative", width: size, height: size, overflow: "hidden", borderRadius: "50%", flex: "0 0 auto" }}><img src={src} alt="" draggable={false} style={{ position: "absolute", width: "178%", height: "178%", left: "-39%", top: "-39%", objectFit: "contain", mixBlendMode: "screen", filter: "brightness(1.06) saturate(.96)", pointerEvents: "none" }} /></span>;
  ...
  return (
    <div data-journey-rank-panel style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 18, minWidth: 0, ...style }}>
      <strong data-character-name style={{ color: "#f8e6c3", fontSize: 30, lineHeight: 1, fontWeight: 950, letterSpacing: ".01em", whiteSpace: "nowrap", textShadow: "0 5px 20px rgba(0,0,0,.28)" }}>{subjectName}</strong>
      <div data-current-rank style={{ display: "flex", alignItems: "center", gap: 9, minWidth: 0, marginLeft: "auto" }}>
        ...
        <strong data-current-rank-name style={{ minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", color: "#f3d18e", fontSize: 17, fontWeight: 950, letterSpacing: ".07em", textTransform: "uppercase", whiteSpace: "nowrap" }}>{current.name}</strong>
      </div>
    </div>
  );
}
