...
export function InventoryScene({ surfaceId = PINORIA_SHOP_SURFACE_ID, companionVariant = "default", characterVariant = "default" }: { surfaceId?: string; companionVariant?: "default" | "egg-water" | "sigil-mori" | "mori-sleep"; characterVariant?: "default" | "secret-butterfly" | "piano-teacher" }) {
        ...
        <section style={{ position: "relative", minHeight: 0, borderRadius: 26, overflow: "hidden", border: "1px solid rgba(236,194,115,.2)", background: "linear-gradient(180deg,rgba(46,30,25,.61),rgba(24,16,14,.78))", boxShadow: "0 30px 60px rgba(0,0,0,.27)" }}>
          ...
          >
            <div style={{ position: "relative", width: "min(440px,34vw,56vh)", aspectRatio: "1 / 1", display: "grid", placeItems: "center" }}>
              {characterVariant === "secret-butterfly" ? <SecretButterflyCharacter markIds={activatedMarkIds} /> : characterVariant === "piano-teacher" ? <PianoTeacherCharacter /> : <PrototypeCharacter subjectId={session?.subject.id ?? "bo"} motion="shop-preview" layerOverrides={layerOverrides} prestigeMarkIds={activatedMarkIds} size="100%" style={{ filter: "drop-shadow(0 19px 22px rgba(0,0,0,.2))" }} />}
