...
export function AmbientHouseRuntime({ learners, departingId = null, suppressedIds = [], frozenIds = [] }: { learners: readonly AmbientHouseLearner[]; departingId?: string | null; suppressedIds?: readonly string[]; frozenIds?: readonly string[] }) {
  ...
  const speedMultipliers = useMemo(() => new Map(learners.map((learner) => [learner.id, resolvePinoriaEffectPresentation(learner.effectKey)?.speedMultiplier ?? 1])), [learners]);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (now - lastCommit.current >= 40) {
        ...
        setAgents((current) => stepAmbientAgents(current, GRAPH, elapsed, { departingIds: departing, frozenIds: frozen, speedMultipliers }));
  ...
  }, [departing, frozenKey, speedMultipliers]);
  ...
  const renderAgent = (agent: AmbientAgent) => {
    ...
    const effect = resolvePinoriaEffectPresentation(learner.effectKey);
    const scale = (0.72 + (agent.y / GRAPH.canvas.height) * 0.28) * (effect?.houseScale ?? 1);
    const style = {
    ...
    return <div key={agent.id} className={styles.agent} style={style} data-pinoria-effect={effect?.key ?? ""} data-effect-speed={effect?.speedMultiplier ?? 1} data-ambient-runtime-character={agent.id} data-ambient-runtime-visit={learner.visitId} data-suppressed={suppressed.has(agent.id) ? "true" : "false"} data-lane={agent.laneId} data-motion-state={agent.motionState} data-connector={agent.connectorId ?? ""} data-departing={departing.has(agent.id) ? "true" : "false"}>
      <LayeredCharacter className={styles.character} config={learner.config} effectKey={learner.effectKey} effectSurface="HOUSE_MINI" />
