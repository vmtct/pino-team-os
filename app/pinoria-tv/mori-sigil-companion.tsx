...
const SIGIL_GLOW_CORE = "hsl(210 50% 28%)";
const SIGIL_GLOW_OUTER = "hsl(210 50% 33%)";
  ...
  return (
    <div data-pinoria-mori-sigil style={{ position: "relative", width: size, aspectRatio: "1 / 1", pointerEvents: "none", ...style }}>
      <div data-pinoria-mori-sigil-breath style={{ position: "absolute", inset: 0, transformOrigin: "50% 50%", animation: "pinoriaMoriSigilBreath 3.8s cubic-bezier(.45,.05,.55,.95) infinite", willChange: "transform" }}>
        ...
        <img data-pinoria-mori-sigil-image src={SIGIL_MORI_URL} alt="" draggable={false} decoding="async" loading="eager" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "50% 50%", filter: `drop-shadow(0 0 10px ${SIGIL_GLOW_CORE}) drop-shadow(0 0 24px ${SIGIL_GLOW_OUTER}) drop-shadow(0 10px 14px rgba(0,0,0,.34))` }} />
      </div>
