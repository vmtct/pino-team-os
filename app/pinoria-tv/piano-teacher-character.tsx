...
export const PIANO_TEACHER_URL = "https://assets.pinohouse.art/pinoria/secret-piano-teacher.webm";
const PIANO_TEACHER_MARK_IDS = ["mark-03"] as const;

export function PianoTeacherCharacter({ style }: { style?: CSSProperties }) {
  return (
    <div data-pinoria-piano-teacher-shell style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
      <style>{`
      ...
      <video data-pinoria-piano-teacher src={PIANO_TEACHER_URL} aria-hidden="true" autoPlay loop muted playsInline preload="auto" disablePictureInPicture style={{ position: "absolute", inset: 0, zIndex: 10, width: "100%", height: "100%", objectFit: "cover", objectPosition: "50% 50%", pointerEvents: "none", filter: "drop-shadow(0 19px 22px rgba(0,0,0,.2))", ...style }} />
    </div>
  );
}