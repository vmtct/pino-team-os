...
type DragTarget = { zone: "outer" | "inner-1" | "inner-2"; index: number } | null;
...
export function AmbientHouseEditor() {
  ...
  function updateZone(zone: "outer" | "inner-1" | "inner-2", updater: (points: AmbientHousePoint[]) => AmbientHousePoint[]) {
    setDraft((current) => {
      const next: Draft = {
        ...current,
      ...
      console.log("NAVMESH_CHANGE", { zone, points: zone === "outer" ? next.outerBoundary : next.obstacles[zone === "inner-1" ? 0 : 1].points });
      return next;
    });
  }

  function activeZone(): "outer" | "inner-1" | "inner-2" | null {
  ...
  function beginVertexDrag(event: React.PointerEvent<SVGCircleElement>, zone: "outer" | "inner-1" | "inner-2", index: number) {
  ...
  const renderVertices = (zone: "outer" | "inner-1" | "inner-2", points: readonly AmbientHousePoint[]) => points.map((point, index) => (
    <g key={`${zone}-${index}`}>
      <circle
        cx={point.x}
        cy={point.y}
    ...
    <div ref={viewportRef} style={{ position: "fixed", inset: 0, overflow: "hidden", background: "#101711", color: "white", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ position: "absolute", left: "50%", top: "50%", width: 1920 * scale, height: 1080 * scale, transform: "translate(-50%, -50%)" }}>
        ...
        >
          ...
          <div data-ambient-mini-character style={{ position: "absolute", left: mini.x, top: mini.y, width: 164, height: 115, zIndex: 30, cursor: mode === "move" ? "grab" : "default", ["--ambient-mini-name" as string]: '"Bơ"' }}>
          ...
          <svg viewBox="0 0 1920 1080" width="1920" height="1080" style={{ position: "absolute", inset: 0, zIndex: 60, overflow: "visible", pointerEvents: "none" }}>
            ...
            {draft.obstacles.map((zone) => <polygon key={zone.id} points={pointList(zone.points)} fill="#ff4f4f35" stroke="#ff6969" strokeWidth="4" />)}
      ...
      <aside style={{ position: "absolute", left: 18, top: 18, zIndex: 100, width: 330, padding: 14, borderRadius: 16, background: "#0d1510e8", border: "1px solid #ffffff22", backdropFilter: "blur(10px)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginTop: 12 }}>
          {zoneButtons.map((button) => <button key={button.id} onClick={() => { setMode(button.id); setSelected(null); }} style={{ padding: "8px 6px", borderRadius: 9, border: "1px solid #ffffff24", background: mode === button.id ? "#f0d076" : "#ffffff10", color: mode === button.id ? "#1d251e" : "#fff", cursor: "pointer", fontWeight: 700 }}>{button.label}</button>)}
        </div>
        ...
        <div style={{ marginTop: 10, fontSize: 12, lineHeight: 1.5 }}>
          <div>Mini x {mini.x} · y {mini.y} · {walkable(mini, draft) ? "✓ walkable" : "× blocked"}</div>
        ...
        <div style={{ marginTop: 8, fontSize: 10, opacity: .58 }}>Edit mode: click empty stage to add a vertex; drag white handles to move. Changes log as NAVMESH_CHANGE and persist locally.</div>
      </aside>
    </div>
  );
}
