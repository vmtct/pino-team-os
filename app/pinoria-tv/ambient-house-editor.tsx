...
export function AmbientHouseEditor() {
  ...
  function onStagePointerDown(event: React.PointerEvent<HTMLDivElement>) {
    ...
    setStatus(`Added ${zoneLabel(zone)} center-anchor point ${point.x}, ${point.y}`);
  }
  ...
  function selectEditMode(nextMode: EditMode) {
    ...
    if (nextMode !== "move" && isAreaZone(nextMode)) {
      ...
      setRoamArea(areaId);
      setStatus(`Editing ${AMBIENT_HOUSE_AREA_LABELS[areaId]} center-anchor home/wander area.`);
    }
  }
        ...
        >
          ...
          {!demo ? (
            <svg viewBox="0 0 1920 1080" width="1920" height="1080" style={{ position: "absolute", inset: 0, zIndex: 60, overflow: "visible", pointerEvents: "none" }}>
              ...
              <text x={mini.x + 13} y={mini.y - 13} fill="#ffe266" fontSize="16" stroke="#000" strokeWidth="3" paintOrder="stroke">
      ...
      {!demo ? (
        <aside style={{ position: "absolute", left: 18, top: 18, zIndex: 100, width: 390, maxHeight: "calc(100vh - 36px)", overflowY: "auto", padding: 14, borderRadius: 16, background: "#0d1510e8", border: "1px solid #ffffff22", backdropFilter: "blur(10px)" }}>
          ...
          <div style={{ marginTop: 10, fontSize: 12, lineHeight: 1.55 }}>
            <div><span style={{ color: "#ffe266" }}>●</span> Anchor CENTER x {mini.x} · y {mini.y} · {anchorGlobalWalkable ? "✓ global" : "× blocked"}</div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
            ...
            <button onClick={enterDemo} style={{ padding: "7px 12px", background: "#f0d076", color: "#162018", border: 0, borderRadius: 8, fontWeight: 800 }}>DEMO</button>
          </div>
        ...
        <button
          ...
          style={{ position: "fixed", right: 18, top: 18, zIndex: 120, padding: "8px 11px", borderRadius: 999, border: "1px solid #ffffff2d", background: "#101711aa", color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer", backdropFilter: "blur(8px)" }}
        >
          EXIT DEMO
        </button>
      )}
