  ...
  return (
    <div
      style={{
        ...
        overflow: "hidden",
        color: "#fff",
        background: "radial-gradient(circle at 68% 40%,#8c956d 0,#495439 34%,#273126 67%,#171e18 100%)",
      }}
    >
      <style>{`
        ...
        @keyframes pinoriaDepartureCharacter {
          0%,7% { opacity:0; transform:translateX(48px) scale(.96) }
          34% { opacity:1; transform:translateX(-2px) scale(1.012) }
          48%,100% { opacity:1; transform:translateX(0) scale(1) }
        }
      ...
      <div
        style={{
          ...
          padding: "72px clamp(64px,7vw,112px) 58px",
          display: "grid",
          gridTemplateColumns: "minmax(0,.94fr) minmax(460px,1.06fr)",
          gap: 42,
          alignItems: "center",
        }}
      >
        <section className="pinoriaDepartureCopy" style={{ maxWidth: 620, animation: "pinoriaDepartureCopy 8.6s cubic-bezier(.18,.8,.2,1) both" }}>
        ...
        <section aria-hidden="true" className="pinoriaDepartureCharacter" style={{ position: "relative", width: "min(600px,47vw)", height: "min(610px,75vh)", justifySelf: "end", display: "grid", placeItems: "center", animation: "pinoriaDepartureCharacter 8.6s cubic-bezier(.18,.8,.2,1) both" }}>
          <div style={{ position: "absolute", width: "78%", aspectRatio: "1", borderRadius: "50%", border: "1px solid rgba(236,216,146,.16)", boxShadow: "0 0 70px rgba(223,205,119,.14),inset 0 0 60px rgba(255,255,255,.025)" }} />
          <div style={{ position: "absolute", left: "22%", right: "8%", bottom: "8%", height: 36, borderRadius: "50%", background: "rgba(4,8,5,.55)", filter: "blur(16px)" }} />

          <PrototypeCharacter
            subjectId={subject.id}
            wingMotion="idle"
            size="min(520px,43vw)"
            style={{ position: "relative", zIndex: 2, marginRight: 18, filter: "drop-shadow(0 30px 30px rgba(0,0,0,.30))", animation: "pinoriaDepartureFloat 3.8s ease-in-out infinite" }}
          />

          <div style={{ position: "absolute", right: "9%", bottom: "11%", zIndex: 4, minWidth: 150, padding: "8px 12px", borderRadius: 999, textAlign: "center", background: "rgba(18,29,20,.84)", border: "1px solid rgba(255,255,255,.12)", boxShadow: "0 12px 26px rgba(0,0,0,.18)", backdropFilter: "blur(9px)" }}>
