...
import { initialAmbientDepthState, resolveAmbientDepthState } from "./ambient-house-motion";
import { PrototypeCharacter } from "./prototype-assets";

const START: AmbientHousePoint = { x: 382, y: 907.5 };
const ASSET_VERSION = "ambient-house-1920-20260821c-motion";
...
type LayerInfo = { state: LayerState; width?: number; height?: number; transparentPct?: number; semiTransparentPct?: number; opaquePct?: number };

function points(items: readonly AmbientHousePoint[]) {
  return items.map((point) => `${point.x},${point.y}`).join(" ");
}

function CanvasHouseLayer({ layer, src, zIndex, onReport }: { layer: LayerName; src: string; zIndex: number; onReport: (layer: LayerName, info: LayerInfo) => void }) {
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      ...
      if (!context) return onReport(layer, { state: "error" });
    ...
    image.onerror = () => { if (!cancelled) onReport(layer, { state: "error" }); };
    image.src = src;
    return () => { cancelled = true; image.onload = null; image.onerror = null; };
  }, [layer, onReport, src]);

  return <canvas ref={canvasRef} width={AMBIENT_HOUSE_CANVAS.width} height={AMBIENT_HOUSE_CANVAS.height} aria-label={`HOUSE ${layer.toUpperCase()}`} style={{ position: "absolute", left: 0, top: 0, width: AMBIENT_HOUSE_CANVAS.width, height: AMBIENT_HOUSE_CANVAS.height, zIndex, pointerEvents: "none", userSelect: "none" }} />;
}
...
export function AmbientHouseScene({ debug = true }: { debug?: boolean }) {
  ...
  const [depthState, setDepthState] = useState(() => initialAmbientDepthState(START, AMBIENT_HOUSE_DEPTH_RULES));
  const [navVisible, setNavVisible] = useState(debug);
  const [layerInfo, setLayerInfo] = useState<Record<LayerName, LayerInfo>>({ back: { state: "loading" }, mid: { state: "loading" }, front: { state: "loading" } });
  ...
  const reportLayer = useMemo(() => (name: LayerName, info: LayerInfo) => {
    setLayerInfo((current) => {
      const existing = current[name];
      if (existing.state === info.state && existing.width === info.width && existing.height === info.height && existing.transparentPct === info.transparentPct && existing.semiTransparentPct === info.semiTransparentPct && existing.opaquePct === info.opaquePct) return current;
  ...
  const charStyle = useMemo(() => ({
    ...
    zIndex: depthState.inFrontOfMid ? 30 : 15,
    filter: "drop-shadow(0 8px 8px rgba(0,0,0,.32))",
    cursor: debug ? "grab" : "default",
    touchAction: "none",
    ["--ambient-mini-name" as string]: '"Bơ"',
  }), [debug, depthState.inFrontOfMid, topLeft.x, topLeft.y]);

  function canonicalPoint(clientX: number, clientY: number) {
    const rect = stageRef.current?.getBoundingClientRect();
    if (!rect || stageScale <= 0) return null;
    return { x: Math.round(((clientX - rect.left) / stageScale) * 10) / 10, y: Math.round(((clientY - rect.top) / stageScale) * 10) / 10 } satisfies AmbientHousePoint;
  }

  function tryMove(clientX: number, clientY: number) {
    const next = canonicalPoint(clientX, clientY);
    if (!next || !isAmbientMiniCharacterAnchorWalkable(next)) return;
    setAnchor(next);
    setDepthState((current) => resolveAmbientDepthState(current, next, AMBIENT_HOUSE_DEPTH_RULES, AMBIENT_HOUSE_INNER_BOUNDARIES));
  }
  ...
  function onPointerMove(event: React.PointerEvent<HTMLDivElement>) { if (debug && draggingRef.current) tryMove(event.clientX, event.clientY); }
  ...
  return (
    <div ref={viewportRef} style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", overflow: "hidden", background: "#101711" }}>
      <div style={{ position: "relative", width: scaledWidth, height: scaledHeight, overflow: "hidden" }}>
        <div ref={stageRef} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={() => { draggingRef.current = false; }} style={{ position: "absolute", left: 0, top: 0, width: AMBIENT_HOUSE_CANVAS.width, height: AMBIENT_HOUSE_CANVAS.height, transform: `scale(${stageScale})`, transformOrigin: "0 0", overflow: "hidden", userSelect: "none", touchAction: "none", background: "#3d4939", isolation: "isolate" }}>
          ...
          <CanvasHouseLayer layer="mid" src={AMBIENT_HOUSE_ASSETS.mid} zIndex={20} onReport={reportLayer} />
          <CanvasHouseLayer layer="front" src={AMBIENT_HOUSE_ASSETS.front} zIndex={50} onReport={reportLayer} />

          {navVisible ? (
            <svg width={AMBIENT_HOUSE_CANVAS.width} height={AMBIENT_HOUSE_CANVAS.height} viewBox={`0 0 ${AMBIENT_HOUSE_CANVAS.width} ${AMBIENT_HOUSE_CANVAS.height}`} style={{ position: "absolute", left: 0, top: 0, width: AMBIENT_HOUSE_CANVAS.width, height: AMBIENT_HOUSE_CANVAS.height, zIndex: 60, pointerEvents: "none" }}>
              <polygon points={points(AMBIENT_HOUSE_OUTER_BOUNDARY)} fill="#5ae38d22" stroke="#72f0a0" strokeWidth="5" />
              {AMBIENT_HOUSE_INNER_BOUNDARIES.map((zone, index) => zone.length >= 2 ? <polygon key={index} points={points(zone)} fill="#ff5c5c22" stroke="#ff7878" strokeWidth="5" strokeDasharray="10 7" /> : null)}
              <line x1="0" y1={AMBIENT_HOUSE_DEPTH_RULES.groundFrontMinYExclusive} x2={AMBIENT_HOUSE_CANVAS.width} y2={AMBIENT_HOUSE_DEPTH_RULES.groundFrontMinYExclusive} stroke="#f7ca66" strokeWidth="3" strokeDasharray="14 10" opacity=".75" />
              <line x1="0" y1={AMBIENT_HOUSE_DEPTH_RULES.upperFrontY} x2={AMBIENT_HOUSE_CANVAS.width} y2={AMBIENT_HOUSE_DEPTH_RULES.upperFrontY} stroke="#77bfff" strokeWidth="3" strokeDasharray="14 10" opacity=".75" />
          ...
          {debug ? (
            <div style={{ position: "absolute", left: 20, top: 20, zIndex: 80, padding: "12px 14px", borderRadius: 14, background: "#101711dd", color: "#f7f0df", fontFamily: "system-ui, sans-serif", fontSize: 13, lineHeight: 1.45, border: "1px solid #ffffff20", pointerEvents: "auto", maxWidth: 440 }}>
              <strong style={{ display: "block", letterSpacing: ".08em", fontSize: 11, color: "#e9c66d" }}>AMBIENT · CENTER ANCHOR · DEPTH LOCK</strong>
              <span>CANVAS 1920×1080 · scale {stageScale.toFixed(4)}</span><br />
              <span>center x {anchor.x.toFixed(1)} · y {anchor.y.toFixed(1)}</span><br />
              <span>{walkable ? "✓ Đi được" : "× Ngoài vùng"} · {depthState.inFrontOfMid ? "CHAR trước MID" : "CHAR sau MID"}</span><br />
              <span>{depthState.lockedInnerBoundary !== null ? `LOCK inner-${depthState.lockedInnerBoundary + 1}` : "depth threshold live"}</span>
              {(Object.entries(layerInfo) as [LayerName, LayerInfo][]).map(([name, info]) => <span key={name} style={{ display: "block", marginTop: 3, fontSize: 11, opacity: .82 }}>{name.toUpperCase()} {info.state}{info.width && info.height ? ` · ${info.width}×${info.height}` : ""}</span>)}
              <button onClick={(event) => { event.stopPropagation(); setNavVisible((value) => !value); }} style={{ marginTop: 8, border: "1px solid #ffffff2a", borderRadius: 999, background: "#ffffff10", color: "inherit", padding: "5px 9px", cursor: "pointer" }}>{navVisible ? "Ẩn navmesh" : "Hiện navmesh"}</button>
            </div>
          ) : null}
        </div>
      </div>
