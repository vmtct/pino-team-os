...
export function AmbientHouseScene({ debug = true }: { debug?: boolean }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", overflow: "hidden", background: "#101711" }}>
      ...
      >
        <HouseLayer src={AMBIENT_HOUSE_ASSETS.back} zIndex={10} label="HOUSE BACK" />

        {charInFrontOfMid ? (
          <HouseLayer src={AMBIENT_HOUSE_ASSETS.mid} zIndex={20} label="HOUSE MID" />
        ) : null}
        ...
        {!charInFrontOfMid ? (
          <HouseLayer src={AMBIENT_HOUSE_ASSETS.mid} zIndex={40} label="HOUSE MID" />
        ) : null}

        <HouseLayer src={AMBIENT_HOUSE_ASSETS.front} zIndex={50} label="HOUSE FRONT" />
