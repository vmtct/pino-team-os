...
export function SyllabusProfileEditor({detail,owners}:{detail:BoLearningSyllabusDetail;owners:BoLearningSyllabusOwnerCatalog}){
  ...
  const [libraryOpen,setLibraryOpen]=useState(false),[libraryQuery,setLibraryQuery]=useState(""),[libraryError,setLibraryError]=useState(""),[libraryLoading,setLibraryLoading]=useState(false),[uploadState,setUploadState]=useState<UploadState>({status:"idle"});
  ...
  async function load(id:string,nextKind:BoLearningSyllabusProfileKind){
    setBusy("load");setError("");setLibraryError("");setLibrary([]);setLibraryOpen(false);setUploadState({status:"idle"});uploadReplay.current=null;
    try{
      const raw=await (nextKind==="ARTCHITECT"?boApi.artSyllabusProfile(id):nextKind==="PIANOHOUSE"?boApi.pianoSyllabusProfile(id):boApi.littlePinerSyllabusProfile(id));
  ...
  async function toggleLibrary(){
    ...
    setLibraryOpen(true);setLibraryError("");setLibraryLoading(true);
    try{setLibrary(await boApi.syllabusWorksheetMedia());}catch(cause){setLibraryError(message(cause));}finally{setLibraryLoading(false);}
  }
  ...
  return <section className={`${styles.panel} ${styles.syllabusProfilePanel}`}>
    ...
    {busy==="load"?<div className={styles.empty}>Loading canonical profile…</div>:<div className={styles.profileEditorGrid}>
      ...
      {needsArt?<section className={`${styles.profileBlock} ${styles.worksheetBlock}`}><div className={styles.profileBlockHeading}><strong>{kind==="LITTLE_PINER"?"B / Worksheets":"Worksheets"}</strong><span>{form.worksheetIds.length}/20 · ordered</span></div>
        ...
        {libraryOpen?<div className={styles.mediaLibrary}><div className={styles.mediaLibraryHead}><div><strong>Media Library</strong><span>Core-valid protected worksheet assets only</span></div><input aria-label="Search Media Library" value={libraryQuery} onChange={e=>setLibraryQuery(e.target.value)} placeholder="Search filename, MIME or ID"/></div>{libraryError?<div className={styles.profileNote}>Media Library unavailable for this BO authority. Existing worksheet references remain editable and saveable.</div>:<div className={styles.mediaLibraryGrid}>{libraryLoading?<div className={styles.empty}>Loading selectable worksheet media…</div>:filteredLibrary.length?filteredLibrary.map(item=><MediaLibraryItem key={item.mediaAssetId} item={item} selected={form.worksheetIds.includes(item.mediaAssetId)} onAdd={()=>addWorksheet(item.mediaAssetId)}/>):<div className={styles.empty}>No selectable worksheet media.</div>}</div>}</div>:null}
        {missingMedia.length&&library.length>0?<div className={styles.profileNote}>{missingMedia.length} existing worksheet reference(s) are outside the currently enumerable Media Library; they remain canonical profile references unless removed or replaced.</div>:null}
