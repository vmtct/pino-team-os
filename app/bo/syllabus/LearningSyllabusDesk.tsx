...
export function LearningSyllabusDesk(){
  ...
  const [owners,setOwners]=useState<BoLearningSyllabusOwnerCatalog>({housePaths:[],toppiPrograms:[]});
  ...
  const ownerOptions=useMemo(()=>createOwnerType==="HOUSE_PATH"?owners.housePaths.map(x=>({id:x.id,label:`${x.displayName} · ${x.code}`})):owners.toppiPrograms.map(x=>({id:x.id,label:x.code.replaceAll("_"," ")})),[createOwnerType,owners]);
  const filterOptions=useMemo(()=>ownerType==="HOUSE_PATH"?owners.housePaths.map(x=>({id:x.id,label:`${x.displayName} · ${x.code}`})):ownerType==="TOPPI_PROGRAM"?owners.toppiPrograms.map(x=>({id:x.id,label:x.code.replaceAll("_"," ")})):[],[ownerType,owners]);
  ...
  async function saveDraft(){const draft=detail?.currentDraft;if(!detail||!draft)return;await run("save",()=>boApi.saveLearningSyllabusDraft(detail.syllabus.id,draft.revision,payload(form)));}
  ...
  return <section className={`${styles.page} ${styles.syllabusPage}`}>
    ...
    <section className={styles.syllabusWorkspace}>
      <aside className={styles.syllabusDirectory}>
        <div className={styles.syllabusToolbar}><label className={styles.field}>Owner type<select value={ownerType} onChange={e=>{const next=e.target.value as "ALL"|BoLearningSyllabusOwnerType;void applyFilter(next,"");}}><option value="ALL">All</option><option value="HOUSE_PATH">House Path</option><option value="TOPPI_PROGRAM">Toppi Program</option></select></label>{ownerType!=="ALL"?<label className={styles.field}>Owner<select value={ownerId} onChange={e=>{setOwnerId(e.target.value);void applyFilter(ownerType,e.target.value);}}><option value="">All {ownerType==="HOUSE_PATH"?"Paths":"Programs"}</option>{filterOptions.map(x=><option key={x.id} value={x.id}>{x.label}</option>)}</select></label>:null}<button className={styles.primaryButton} onClick={()=>{setCreateOpen(x=>!x);setForm(blankDraft);}}>Create Syllabus</button></div>
