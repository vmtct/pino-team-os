...
export default function PinerPrototypeV3() {
  ...
  return (
    <main className={`${base.prototype} ${v3.prototypeV3}`}>
      <aside className={base.labPanel}>
        <div className={base.labBadge}>LOCAL PROTOTYPE · V3</div>
        <h1>Piner Member Space</h1>
        <p>Mock data only. V3 tập trung schedule context, centered syllabus rail, Explore CMS campaigns và item-level Collection gating.</p>
        ...
        <div className={base.labNotes}>
          <strong>V3 review focus</strong>
          <span>Current package shows DOW + class time.</span>
          <span>Syllabus rail scrolls locally and centers Current.</span>
          <span>26B08 is an opaque occurrence code, not parent-facing W8.</span>
          <span>Collection groups by outcome; access can lock inside one item.</span>
          <span>Explore campaign items are TOS-managed CMS projections.</span>
        </div>
      </aside>
      ...
      <section className={base.deviceStage}>
        <div className={base.device}>
          ...
          <div className={base.screen}>
            ...
            {activeTab === "journey" && <JourneyScreen student={student} activePath={activePath} setActivePath={setActivePath} onPremium={() => setOverlay({ type: "premium" })} />}
          ...
          {overlay && (
            <div className={base.overlayBackdrop} onMouseDown={() => setOverlay(null)}>
              <div className={base.sheet} onMouseDown={(event) => event.stopPropagation()}>
...
function PianoJourney({ expired, onPremium }: { expired: boolean; onPremium: () => void }) {
  return (
    <>
      ...
      <section className={base.sectionBlock}><div className={base.sectionHeading}><div><span className={base.eyebrow}>Practice support</span><h3>Resources theo progress</h3></div></div><div className={base.resourceGrid}><div><span>♩</span><strong>Sheet</strong><small>Verse 1 + Chorus</small></div><div><span>☝</span><strong>Hand map</strong><small>Two-hand guide</small></div><div><span>▶</span><strong>Listen</strong><small>Reference audio</small></div><div className={base.resourceLocked}><span>◇</span><strong>Expansion</strong><small>{expired ? "Locked by access" : "Mở từ L6"}</small></div></div></section>
    </>
  );
}

function LittlePinerPianoJourney() {
  return <><section className={base.journeyHero}><span className={base.eyebrow}>Starter collection</span><div className={base.journeyHeroRow}><div><h3>ABC Song</h3><p>Self-paced · independent LPP definition</p></div><span className={base.bigGlyph}>♫</span></div></section><section className={base.sectionBlock}><div className={base.sectionHeading}><div><span className={base.eyebrow}>Songs</span><h3>Những bài đang giữ</h3></div></div><div className={base.songList}><div className={base.songCurrent}><span>🎵</span><strong>ABC Song</strong><small>Current · worksheet mở</small></div><div><span>♪</span><strong>Twinkle Twinkle</strong><small>Available</small></div><div className={base.songLocked}><span>◇</span><strong>Next familiar song</strong><small>Future visibility</small></div></div></section></>;
}
...
function ExploreScreen({ student, campaignIndex, setCampaignIndex, onCampaign, onPremium }: {
  ...
  return (
    <div className={base.stack}>
      ...
      <section className={`${base.eligibilityCard} ${blocked ? base.eligibilityBlocked : ""}`}><span className={base.eyebrow}>{student.membershipLabel}</span><h3>{blocked ? "Tuần này đã có Open Studio được xác nhận" : student.exploreStatus === "premium" ? "Explore benefits đang khả dụng" : "Tuần này có thể khám phá"}</h3><p>{student.exploreNote}</p>{blocked && <button type="button" className={base.primaryButton} onClick={onPremium}>Khám phá quyền lợi Premium →</button>}</section>
